...
class DiagramToolbar extends DiagramPanel {
  ...
  _renderToolbar($toolbar) {
    var beforeCommands = this._commands.filter(command => ['after', 'center'].indexOf(command.position) === -1);

    var centerCommands = this._commands.filter(command => command.position === 'center');

    var afterCommands = this._commands.filter(command => command.position === 'after');
  ...
  _createSelectBoxItemOptions(command, hint, items, valueExpr, displayExpr) {
    ...
    var isSelectButton = items && items.every(i => i.icon !== undefined);
