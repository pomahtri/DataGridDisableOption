...
var createAgendaAppointmentLayout = function createAgendaAppointmentLayout(formatText, config) {
  ...
  config.isRecurrence && (0, _renderer.default)('<span>').addClass(_constants.APPOINTMENT_CONTENT_CLASSES.RECURRING_ICON + ' dx-icon-repeat').appendTo(marker); // eslint-disable-next-line no-unused-vars

  var text = (0, _renderer.default)('<div>').addClass(_constants.APPOINTMENT_CONTENT_CLASSES.APPOINTMENT_TITLE).text(formatText.text).appendTo(rightLayoutContainer);
  var additionalContainer = (0, _renderer.default)('<div>').addClass(_constants.APPOINTMENT_CONTENT_CLASSES.APPOINTMENT_CONTENT_DETAILS).appendTo(rightLayoutContainer); // eslint-disable-next-line no-unused-vars

  var date = (0, _renderer.default)('<div>').addClass(_constants.APPOINTMENT_CONTENT_CLASSES.APPOINTMENT_DATE).text(formatText.formatDate).appendTo(additionalContainer);
